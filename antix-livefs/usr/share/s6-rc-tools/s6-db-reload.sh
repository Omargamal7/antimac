...
load_conf() {
    conf="${SYSPATH}/config/s6-db-reload.conf"
    [ -f "$conf" ] || return 1
    # shellcheck source=/etc/s6-rc/config/s6-db-reload.conf
    [ -r "$conf" ] && . "$conf"

    TIMESTAMP=${TIMESTAMP:-@$(date +%Y%m%d-%h%m%s)}

    USERPATH=${USERPATH:-${XDG_DATA_HOME:-$HOME}}
...
check_db() {
	if [ "${USERDB}" -eq 1 ]; then
		_args="${DBPATH}-${TIMESTAMP} ${SVPATH} ${USERSVPATH}"
    else
		_args="${DBPATH}-${TIMESTAMP} ${SVPATH} ${ADMINSVPATH}"
    fi
    # shellcheck disable=SC2086
