...
VERSION="1.00"
VDATE="Sun 09 Jun 2019 01:32:11 PM MDT"
...
CMDLINE_ARGS=("$@")
 ...
 TERM_TITLE_OPT="--title"
     GUI_FILERS="rox thunar spacefm zzzfm dolphin"
...
check_for_yad() {
    ...
    vmsg "$(printf "No yad found.")"
    for arg in ${CMDLINE_ARGS[@]}; do
        args="$args $arg"
    done
    vmsg "exec $0 $args --cli"
    clean_up
    exec $0 $args --cli
    exit
}
...
clean_up() {
    ...
    for luks in ${LUKS_CLOSE}; do
        /usr/sbin/cryptsetup luksClose $luks
    done
